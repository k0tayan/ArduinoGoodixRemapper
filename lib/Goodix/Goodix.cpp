...
void Goodix::onIRQ() {
  ...
  if (contacts >= 0) {
    for(int i=0; i<contacts; i++) {
      uint8_t tmp = i*8;
      points[i].trackId = raw_points[tmp];
      points[i].x = ((uint16_t)raw_points[tmp+2] << 8) + raw_points[tmp+1];
      points[i].y = ((uint16_t)raw_points[tmp+4] << 8) + raw_points[tmp+3];
      points[i].area = ((uint16_t)raw_points[tmp+6] << 8) + raw_points[tmp+5];
    }
